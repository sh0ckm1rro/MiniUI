...
function DCP:CreateOptionsFrame()
    local sliders = {
        { text = "圖示大小", value = "iconSize", min = 30, max = 125, step = 5 },
        { text = "淡入時間", value = "fadeInTime", min = 0, max = 1.5, step = 0.1 },
        { text = "淡出時間", value = "fadeOutTime", min = 0, max = 1.5, step = 0.1 },
        { text = "最大不透明度", value = "maxAlpha", min = 0, max = 1, step = 0.1 },
        { text = "最大不透明度持續時間", value = "holdTime", min = 0, max = 1.5, step = 0.1 },
        { text = "動畫縮放", value = "animScale", min = 0, max = 2, step = 0.1 },
    }

    local buttons = {
        { text = "關閉", func = function(self) self:GetParent():Hide() end },
        { text = "測試", func = function(self)
    ...
    ignoretypetextblacklist:SetText("黑名單")
    ...
    ignoretypetextwhitelist:SetText("白名單")
    ...
    ignorebox:SetScript("OnEnter",function(self) GameTooltip:SetOwner(self, "ANCHOR_CURSOR") GameTooltip:SetText("注意：用逗號分隔多個法術") end)
